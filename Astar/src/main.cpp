...
int main(int argc, char * argv[])
{
    ...
    ROS_INFO("Start Fucking astar node!\n");
    ...
    nh_priv.param<int>("namespace", robot_name, 0);

    ROS_INFO("namespace: %d", robot_name);

    // Subscribe topics
    map_sub = nh.subscribe("/map", 10, MapCallback);

    if (robot_name == 0){
        startPoint_sub = nh.subscribe("/tb3_0/my_initialpose", 10, StartPointCallback);
        targetPoint_sub = nh.subscribe("/tb3_0/move_base_my/goal", 10, TargetPointtCallback);
    }
    else if (robot_name == 1){
        startPoint_sub = nh.subscribe("/tb3_1/my_initialpose", 10, StartPointCallback);
        targetPoint_sub = nh.subscribe("/tb3_1/move_base_my/goal", 10, TargetPointtCallback);
    }
    else if (robot_name == 2){
        startPoint_sub = nh.subscribe("/tb3_2/my_initialpose", 10, StartPointCallback);
        targetPoint_sub = nh.subscribe("/tb3_2/move_base_my/goal", 10, TargetPointtCallback);
    }
    ...
    if (robot_name == 0){
        path_pub = nh.advertise<nav_msgs::Path>("/tb3_0/astar_nav_path", 10);
    }
    else if (robot_name == 1){
        path_pub = nh.advertise<nav_msgs::Path>("/tb3_1/astar_nav_path", 10);
    }
    else if (robot_name == 2){
        path_pub = nh.advertise<nav_msgs::Path>("/tb3_2/astar_nav_path", 10);
    }
    ...
    while(ros::ok())
    {
        if(start_flag)
        {
            ...
            if(!PathList.empty())
            {
                ...
                ROS_INFO("Find a tb3_%d's valid path successfully! Use %f s", robot_name, end_time - start_time);
            }
            else
            {
